...
HRESULT CD3DFont::Print(float x, float y, DWORD color, const std::string strText, bool isShadow)
{
	if (!m_isReady)
		return E_FAIL;
...
HRESULT CD3DFont::PrintShadow(float x, float y, DWORD color, const std::string strText)
{
	DWORD shadow = D3DCOLOR_ARGB((BYTE)HIBYTE(HIWORD(color)), 0, 0, 0);
	Print(x + 1, y + 1, shadow, strText, true);
	Print(x, y, color, strText);
...
void CD3DRender::D3DGradient(int x, int y, int w, int h, D3DCOLOR rcolor, D3DCOLOR lcolor)
{
...
void CD3DRender::D3DCircle(int x, int y, int radius, D3DCOLOR color, bool filled)
{
	if (SUCCEEDED(Begin(filled ? D3DPT_TRIANGLEFAN : D3DPT_LINESTRIP))) {
		D3DColor(color);
		D3DVertex2f(x + radius, y);
		for (int i = 0; i <= 90; i++)
			D3DVertex2f(x + radius * cosf(D3DX_PI * (i / 45)), y - (filled ? -radius : radius) * sinf(D3DX_PI * (i / 45)));
		End();
	}
}
