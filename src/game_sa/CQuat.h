...
public:
	...
	CQuat(CMatrix* m)
	{
		w = sqrt(max((float)0, 1.0f + m->vRight.fX + m->vFront.fY + m->vUp.fZ)) * 0.5f;
		x = sqrt(max((float)0, 1.0f + m->vRight.fX - m->vFront.fY - m->vUp.fZ)) * 0.5f;
		y = sqrt(max((float)0, 1.0f - m->vRight.fX + m->vFront.fY - m->vUp.fZ)) * 0.5f;
		z = sqrt(max((float)0, 1.0f - m->vRight.fX - m->vFront.fY + m->vUp.fZ)) * 0.5f;
