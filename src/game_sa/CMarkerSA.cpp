...
VOID CMarkerSA::SetColor(eMarkerColor color)
{
	...
	if (color >= MARKER_COLOR_PLUM && color <= MARKER_COLOR_DARK_TURQUOISE)
	{
		...
		{
			internalInterface->bBright = TRUE;
			internalInterface->nColour = eMarkerColor(color - MARKER_COLOR_RED);
		}
		else
		{
			internalInterface->bBright = FALSE;
...
VOID CMarkerSA::SetColor(RGBA color)
{
	...
	internalInterface->nColour = eMarkerColor((DWORD)color);
}
...
eMarkerType CMarkerSA::GetBlipType()
{
	return (eMarkerType)internalInterface->BlipType;
}

eMarkerSprite CMarkerSA::GetSprite()
{
	return (eMarkerSprite)internalInterface->nBlipSprite;
}

eMarkerDisplay CMarkerSA::GetDisplay()
{
	return (eMarkerDisplay)internalInterface->nBlipDisplayFlag;
}

BOOL CMarkerSA::IsBright()
{
	return internalInterface->bBright;
}