...
public:
    ...
    SString& vFormat ( const char* szFormat, va_list vl )
    {
        ...
        // Try to format the std::string into the std::string buffer. If we will need
        ...
        {
            // We didn't have enough capacity to fit the std::string. Count how much
            ...
            else
            {
                ...
                // Grab the new data for the resized std::string.
                curCapacity = std::string::capacity ();
        ...
        // If there weren't any errors, give the formatted std::string back to std::string.
        if ( iSize > -1 )
        {
            szDest [ iSize ] = '\0';
