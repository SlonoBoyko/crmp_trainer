...
CGameSA::CGameSA()
{
    ...
    DWORD oldProt;
    VirtualProtectEx(GetCurrentProcess(), (LPVOID)0x401000, 0x4A3000, PAGE_EXECUTE_READWRITE, &oldProt);
...
{
    std::map < string, BYTE* >::iterator it = m_Cheats.find ( szCheatName );
    if ( it == m_Cheats.end () )
        return false;
    return *(it->second) != 0;
}
...
{
    std::map < string, BYTE* >::iterator it;
