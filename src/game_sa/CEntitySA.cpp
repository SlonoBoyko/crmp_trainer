...
VOID CEntitySA::Teleport(float fX, float fY, float fZ)
{
	...
	if (m_pInterface->Placeable.matrix)
	{
		...
		((void(__thiscall *)(void*, float, float, float, int))(m_pInterface->vtbl->Teleport))(m_pInterface, fX, fY, fZ, 0);
...
CVector * CEntitySA::GetRoll(CVector * vecRoll)
{
	if (m_pInterface->Placeable.matrix && vecRoll) {
		memcpy(vecRoll, &m_pInterface->Placeable.matrix->vRight, sizeof(CVector));
		return vecRoll;
	} else return NULL;
}

CVector * CEntitySA::GetDirection(CVector * vecDir)
{
	if (m_pInterface->Placeable.matrix && vecDir) {
		memcpy(vecDir, &m_pInterface->Placeable.matrix->vFront, sizeof(CVector));
		return vecDir;
	} else return NULL;
}

CVector * CEntitySA::GetWas(CVector * vecWas)
{
	if (m_pInterface->Placeable.matrix && vecWas) {
		memcpy(vecWas, &m_pInterface->Placeable.matrix->vUp, sizeof(CVector));
		return vecWas;
	} else return NULL;
}
...
void CEntitySA::SetIsLocked(bool bLocking)
{
	bLocking ? m_pInterface->nImmunities |= 2 : m_pInterface->nImmunities &= ~2;
};


bool CEntitySA::IsLocked()
{
	return (m_pInterface->nImmunities & 0x2) ? true : false;
};
